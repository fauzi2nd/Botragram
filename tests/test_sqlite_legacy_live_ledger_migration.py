...
def test_does_not_merge_legacy_ledger_into_existing_durable_target() -> None:
    """Protect an independent TESTNET deployment from a legacy-state overwrite."""
    asyncio.run(_run_existing_target_ledger_test())


async def _run_existing_target_ledger_test() -> None:
    """Leave target state untouched when it already has a durable ledger."""
    with TemporaryDirectory() as tmp:
        root = Path(tmp)
        source = SQLiteDatabase(database_path=root / "botragram.db")
        target = SQLiteDatabase(
