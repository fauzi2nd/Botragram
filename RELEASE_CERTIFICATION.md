...
12. `botragram/exchanges/bitget/futures_client.py` & `cfd_client.py`: Replaced `except Exception:` with `except ExchangeOrderNotFoundError:` during `cancel_protection_order()` client-to-order-ID resolution to avoid swallowing unexpected network or cancellation exceptions.
